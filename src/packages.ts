...
export function extractNodeModulesPackageName(uri: string): string | undefined {
  const match = decodeURIComponent(url.parse(uri).pathname || '').match(
    NODE_MODULES_PACKAGE_NAME_REGEXP
  );
...
export class PackageManager extends EventEmitter implements Disposable {
  ...
  constructor(
    ...
    this.subscriptions.add(
      Observable.fromEvent<[string, string]>(
  ...
  public on(
    event: 'parsed',
    listener: (uri: string, packageJson: PackageJson) => void
  ): this;
  ...
  /**
   * Gets the content of the closest package.json known to to the DependencyManager
   * in the ancestors of a URI
   *
   * @return Observable that emits a single PackageJson or never
   */
  public getClosestPackageJson(uri: string): Observable<PackageJson> {
    return this.updater.ensureStructure().concat(
      Observable.defer(() => {
        ...
        return this.getPackageJson(packageJsonUri);
      })
    );
  }
   ...
   * @return Observable that emits a single PackageJson or never
   */
  public getPackageJson(uri: string): Observable<PackageJson> {
    ...
    return this.updater.ensure(uri).concat(
      Observable.defer(() => {
        packageJson = this.packages.get(uri)!;
        if (!packageJson) {
          return Observable.throw(
            new Error(`Expected ${uri} to be registered in PackageManager`)
          );
        }
        return Observable.of(packageJson);
      })
  ...
  /**
   * Walks the parent directories of a given URI to find the first package.json
   * that is known to the InMemoryFileSystem
   *
  ...
  public getClosestPackageJsonUri(uri: string): string | undefined {
    ...
    while (true) {
      ...
      const packageJsonUri = url.format({
        ...parts,
        pathname: path.posix.join(parts.pathname, 'package.json'),
      });
