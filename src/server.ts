...
export function serve(
  ...
  createLangHandler = (remoteClient: RemoteLanguageClient) =>
    new MetaschemaService(remoteClient)
): void {
  ...
  if (options.clusterSize > 1 && cluster.isMaster) {
    ...
    const server = net.createServer(socket => {
      ...
      const messageEmitter = new MessageEmitter(
        socket as NodeJS.ReadableStream,
        options
      );
      const messageWriter = new MessageWriter(socket, options);
      const remoteClient = new RemoteLanguageClient(
        messageEmitter,
        messageWriter
      );
    ...
    server.listen(options.lspPort, () => {
      logger.info(
        `Listening for incoming LSP connections on ${options.lspPort}`
      );
    });
  }
}
