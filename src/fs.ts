...
export class RemoteFileSystem implements FileSystem {
   ...
   * parameter, if given. A language server can use the result to index files
   * by filtering and doing a content request for each text document of
   * interest.
   */
...
export class FileSystemUpdater {
  ...
  /**
   * Fetches the file content for the given URI and adds the content to the
  ...
  public fetch(uri: string): Observable<never> {
    ...
    const observable = Observable.fromPromise(this.concurrencyLimit.wait())
      ...
      .do(
        ...
        err => {
          this.logger.error('Failed to fetch file', uri, err);
  ...
  /**
   * Returns a promise that is resolved when the given URI has been fetched (at
   * least once) to the in-memory file system. This function cannot be
   * cancelled because multiple  callers get the result of the same operation.
   *
  ...
  /**
   * Fetches the file/directory structure for the given directory from the
   * remote  file system and saves it in the in-memory file system
   */
  public fetchStructure(): Observable<never> {
    const observable = this.remoteFs
      .getWorkspaceFiles(undefined)
      .do(
        uri => this.inMemoryFs.add(uri),
        err => {
          this.logger.error('Failed to fetch structure', err);
  ...
  /**
   * Returns a promise that is resolved as soon as the file/directory structure
   * for the given directory has been synced from the remote file system to the
   * in-memory file system (at least once)
   */
