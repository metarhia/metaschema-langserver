...
export interface LanguageClient {
   ...
   * current content of any text document. This allows language servers to
   * operate without accessing the file system directly.
   */
  textDocumentXcontent(params: TextDocumentContentParams): Observable<TextDocumentItem>;

  /**
   * The files request is sent from the server to the client to request a list
   * of all files in the workspace or inside the directory of the `base`
   * parameter, if given.
   */
  ...
  /**
   * The cache get request is sent from the server to the client to request the
   * value of a cache item identified by the provided key.
   */
  xcacheGet(params: CacheGetParams): Observable<any>;

  /**
   * The cache set notification is sent from the server to the client to set
   * the value of a cache item identified by the provided key. This is a
   * intentionally notification and not a request because the server is not
   * supposed to act differently if the cache set failed.
   */
  xcacheSet(params: CacheSetParams): void;

  /**
   * Diagnostics are sent from the server to the client to notify the user of
   * errors/warnings in a source file
   ...
   */
  workspaceApplyEdit(params: ApplyWorkspaceEditParams): Observable<ApplyWorkspaceEditResponse>;
}
...
export class RemoteLanguageClient {
   ...
   * operate without accessing the file system directly.
   */
  public textDocumentXcontent(params: TextDocumentContentParams): Observable<TextDocumentItem> {
   ...
   * value of a cache item identified by the provided key.
   */
  public xcacheGet(params: CacheGetParams): Observable<any> {
    return this.request('xcache/get', params);
  }
  ...
  public workspaceApplyEdit(
    ...
    return this.request('workspace/applyEdit', params);
  }
  ...
  private request(method: string, params: any[] | { [attr: string]: any }): Observable<any> {
    return new Observable<any>(subscriber => {
      ...
      const message: RequestMessage & HasMeta = {
        jsonrpc: '2.0',
        method,
        id,
        params,
      ...
      const messageSub = Observable.fromEvent<Message>(this.input, 'message')
        ...
        .filter((msg): msg is ResponseMessage => isResponseMessage(msg) && msg.id === id)
        .take(1)
